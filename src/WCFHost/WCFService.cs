  ...
  public class WCFService : IWCFService
  {
    ...
    public WCFService()
    {
      ...
      try
      {
        ...
        var objectList = JsonConvert.DeserializeObject<Dictionary<int, ServerObject>>(File.ReadAllText("ObjectsData.json"));
        objects = objectList.Where(o => o.Value.BodyType.Value == ServerObject.BodyTypes.Car).ToDictionary(o => o.Key, o => o.Value);
      }
      ...
      saveTimer.Elapsed += (s, e) =>
      {
        if (players.Count > 0)
        {
          ...
          File.WriteAllText("ObjectsData.json", JsonConvert.SerializeObject(ob.Where(o => o.Value.BodyType.Value == ServerObject.BodyTypes.Car).ToDictionary(o => o.Key, o => o.Value)));
        }
      };
      saveTimer.Start();
      #endregion
    ...
    public void SendPosition(string position)
    {
      ...
      {
        var positions = JsonConvert.DeserializeObject<List<PositionData>>(position);
        foreach (var pos in positions)
        {
          if (pos.Key.HasValue) objects[pos.Key.Value].UpdateObject(pos);
          else players[loggedUsers[id].Name].UpdateObject(pos);
        }
      }
    }
    ...
    /// </summary>
    /// <param name="position">Serialized KeyValuePair contains Position and angle of the player.</param>
    public void CreateBullet(string position)
    {
      string id = OperationContext.Current.SessionId;
      if (loggedUsers.ContainsKey(id))
      {
        var pos = JsonConvert.DeserializeObject<PositionData>(position);
        var obj = new Bullet(world, contentManager.Load<Texture2D>("Materials/bullet"), pos.Position, pos.Angle);
        ServerObject so = new ServerObject(obj, pos.Position, pos.Angle, true, ServerObject.BodyTypes.Bullet, pos.Key);
        objects.Add(pos.Key.Value, so);
      }
    }
