...
namespace WCFReference.Objects
{
  public class Car : GameObject, IPlayable
  {
    ...
    public override void UpdatePosition(Vector2 position, float? angle)
    {
      base.UpdatePosition(position, angle);
      Body.Awake = true;
      if (angle.HasValue)
      {
        Wheels[0].Angle = angle.Value;
        Wheels[1].Angle = angle.Value;
      }
    }
